...
public class MinecraftDisplayer implements IDisplayer {
    ...
    public String randomTooltip(String currentTooltip) {
        ...
        //BetterLoadingScreen.log.trace("Does alreadyUsedTooltips contain res?: "+String.valueOf(alreadyUsedTooltips.contains(res)));
    ...
    public static String[] readTipsFile(String file) throws IOException {
        ...
        try {
            reader = new BufferedReader((new InputStreamReader(new FileInputStream(file), StandardCharsets.UTF_8)));//new BufferedReader(new FileReader(file));
    ...
    public void open(Configuration cfg) {
        ...
        comment4 = "What font texture to use? Special Cases:" + n +
                " - If you use the Russian mod \"Client Fixer\" then change this to \"textures/font/ascii_fat.png\"" + n +
        ...
        String comment7 = "Logo coordinates in image and position." + n + 
                "the first four values indicate where the logo is located on the image (you could use a spritesheet)," + n + 
                "the four next ones tell where the image will be located on screen like this:" + n +
                "[xLocation, yLocation, xWidth, yWidth, xLocation, yLocation, xWidth, yWidth]" + n + 
        		"The same is used for other images, except the background, which is fullscreen. Please ALWAYS provide" + n +
        ...
        try {
            ...
            //BetterLoadingScreen.log.debug("The color: " + String.valueOf(lbRGB[0]) + ";" + String.valueOf(lbRGB[1]) + ";" + String.valueOf(lbRGB[2]));
        } catch (Exception e) {
            lbRGB[0] = 1;
            lbRGB[1] = 0.5176471f;
            lbRGB[2] = 0;
